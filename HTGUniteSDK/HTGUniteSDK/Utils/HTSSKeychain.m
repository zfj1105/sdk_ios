...
NSString *const kHTSSKeychainErrorDomain = @"com.samsoffes.sskeychain";

NSString *const kHTSSKeychainAccountKey = @"acct";
NSString *const kHTSSKeychainCreatedAtKey = @"cdat";
NSString *const kHTSSKeychainClassKey = @"labl";
NSString *const kHTSSKeychainDescriptionKey = @"desc";
NSString *const kHTSSKeychainLabelKey = @"labl";
NSString *const kHTSSKeychainLastModifiedKey = @"mdat";
NSString *const kHTSSKeychainWhereKey = @"svce";

#if __IPHONE_4_0 && TARGET_OS_IPHONE  
CFTypeRef HTSSKeychainAccessibilityType = NULL;
#endif

@interface HTSSKeychain ()
+ (NSMutableDictionary *)_queryForService:(NSString *)service account:(NSString *)account;
@end

@implementation HTSSKeychain
...
+ (NSArray *)accountsForService:(NSString *)service error:(NSError **)error {
    ...
    if (status != noErr && error != NULL) {
		*error = [NSError errorWithDomain:kHTSSKeychainErrorDomain code:status userInfo:nil];
		return nil;
	}
...
#if __IPHONE_4_0 && TARGET_OS_IPHONE
		if (HTSSKeychainAccessibilityType) {
#if __has_feature(objc_arc)
...
+ (CFTypeRef)accessibilityType {
	return HTSSKeychainAccessibilityType;
}
...
+ (void)setAccessibilityType:(CFTypeRef)accessibilityType {
	...
	if (HTSSKeychainAccessibilityType) {
		CFRelease(HTSSKeychainAccessibilityType);
	}
	HTSSKeychainAccessibilityType = accessibilityType;
}
#endif
