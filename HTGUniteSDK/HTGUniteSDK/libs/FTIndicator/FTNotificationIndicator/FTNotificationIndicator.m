...
#define kFTNotificationMargin     60
        ...
        switch (sender.state) {
            case UIGestureRecognizerStateBegan: case UIGestureRecognizerStateChanged:
                if (translation.y < 0) {
                    ...
                    if (kFTScreenWidth >= kFTScreenHeight) {
                        [self.notificationView setFrame:CGRectMake(kFTScreenWidth/4,translation.y,kFTScreenWidth/2,self.notificationView.frame.size.height)];
                    } else {
                        [self.notificationView setFrame:CGRectMake(kFTNotificationMargin,translation.y,kFTScreenWidth - kFTNotificationMargin * 2,self.notificationView.frame.size.height)];
                    }
...
- (void)adjustIndicatorFrame
{
    ...
    if (kFTScreenWidth >= kFTScreenHeight) {
        [self.notificationView setFrame:CGRectMake(KSCREEN_WIDTH/4,- (notificationSize.height),kFTScreenWidth/2,notificationSize.height)];
    } else {
        [self.notificationView setFrame:CGRectMake(kFTNotificationMargin,- (notificationSize.height),kFTScreenWidth - kFTNotificationMargin * 2,notificationSize.height)];
    }
         ...
         usingSpringWithDamping:0.5
          initialSpringVelocity:0
                     ...
                     animations:^{
                         ...
                         if (kFTScreenWidth >= kFTScreenHeight) {
                             [self.notificationView setFrame:CGRectMake(KSCREEN_WIDTH/4,0,kFTScreenWidth/2,self.notificationView.frame.size.height)];
                         } else {
                             [self.notificationView setFrame:CGRectMake(kFTNotificationMargin,0,kFTScreenWidth-kFTNotificationMargin *2,self.notificationView.frame.size.height)];
                         }
...
- (void)dismissingNotificationtViewByTap:(BOOL)tap
{
    ...
    [UIView animateWithDuration:kFTNotificationDefaultAnimationDuration
                     ...
                     animations:^{
                         ...
                         if (kFTScreenWidth >= kFTScreenHeight) {
                             [self.notificationView setFrame:CGRectMake(KSCREEN_WIDTH/4,- (self.notificationView.frame.size.height),kFTScreenWidth/2,(self.notificationView.frame.size.height))];
                         } else {
                             [self.notificationView setFrame:CGRectMake(kFTNotificationMargin,- (self.notificationView.frame.size.height),kFTScreenWidth - kFTNotificationMargin *2,(self.notificationView.frame.size.height))];
                         }
...
- (void)showWithImage:(UIImage *)image title:(NSString *)title message:(NSString *)message style:(UIBlurEffectStyle)style
{
    ...
    _iconImageView.frame = CGRectMake(kFTNotificationMargin_X+15, kFTNotificationStatusBarHeight + kFTNotificationMargin_Y, kFTNotificationImageSize, kFTNotificationImageSize);
    
    self.titleLabel.frame = CGRectMake(kFTNotificationMargin_X*9, kFTNotificationStatusBarHeight, kFTScreenWidth - kFTNotificationMargin_X - text_X,  kFTNotificationTitleHeight);
    self.messageLabel.frame = CGRectMake(kFTNotificationMargin_X*9, kFTNotificationStatusBarHeight+kFTNotificationTitleHeight,  messageSize.width, messageSize.height);
...
{
    CGFloat textWidth = image ? (kFTScreenWidth - kFTNotificationMargin *2 - kFTNotificationMargin_X*11) : (kFTScreenWidth - kFTNotificationMargin_X*2);
    
    if (kFTScreenWidth >= kFTScreenHeight) {
        textWidth = image ? (kFTScreenWidth/2 - kFTNotificationMargin_X*10) : (kFTScreenWidth - kFTNotificationMargin_X*2);
    } else {
        textWidth = image ? (kFTScreenWidth - kFTNotificationMargin *2 - kFTNotificationMargin_X*11) : (kFTScreenWidth - kFTNotificationMargin_X*2);
    }
